/************************************************************************
*   Lagging DG Sizing and Location using Analytic Methods		*
*								        * 
*  Date of Written : 18.03.2017					        *
...
*									*
*  Last Modification Date : 28.04.2017					*
*************************************************************************/
...
typedef struct {
  ...
  char inputfile2[20];		// Annual load pattern data
  char inputfile3[20];		// Read input price details for every load hour
}FileDetails;
typedef struct {
  int bno;
  int bcode_i;
...
FILE* BaseCaseFileWriting(FILE* fb, BaseCaseResult* B, Rect* Volt, Rect* Sinj, BusData* bus, LineData* line, Rect** S, Rect* LineLoss, Polar* V, SystemDetails s, int n) {
  int i, j;
  ...
  fprintf(fb,"\nMinimum voltage in the %d bus system is at bus no : %d\tVoltage magnitude is : %lf" , s.nbus, B[n-1].Vmin_bus, B[n-1].Vmin);
  ...
  fprintf(fb,"\n-----------------------------------------------------------------------------------------\n");
  //fprintf(fb, "Total\t\t\t\t%lf    %lf    %10.3lf    %10.3lf", SumPinj, SumQinj, SumPload, SumQload);
  fprintf(fb,"\n-----------------------------------------------------------------------------------------\n");
  
  fprintf(fb,"\n-----------------------------------------------------------------------------------------");
  fprintf(fb,"\n\t\t\tLine Flow Result\t\t\n");
  fprintf(fb,"-----------------------------------------------------------------------------------------\n");
  fprintf(fb,"| From Bus | To Bus |    P (MW)    |    Q (MVAr)     |\n");
  ...
  fprintf(fb,"\n-----------------------------------------------------------------------------------------\n");
  //fprintf(fb, "Total\t\t\t%lf MW\t%lf MVAr", fabs(SumPLoss), fabs(SumQLoss));
  fprintf(fb,"\n-----------------------------------------------------------------------------------------\n");
  fprintf(fb,"\n#########################################################################################\n");
  
  return fb;     
}
BaseCaseResult* ComputeBaseCaseResult(BaseCaseResult* B, BusData* bus, LineData* line, Rect* LineLoss, Polar* V, SystemDetails s, int n) {
  double SumPload, SumQload, SumPLoss, SumQLoss, minVol;
  int i, j, busno;
  
  SumPload = 0; SumQload = 0; SumPLoss = 0;  SumQLoss = 0;
  ...
  B[n-1].Pload = SumPload;
  B[n-1].Qload = SumQload;
  ...
  B[n-1].Pload = SumPload;
  B[n-1].Qload = SumQload;
  B[n-1].Ploss = fabs(SumPLoss*1000);
  B[n-1].Qloss = fabs(SumQLoss*1000);
  ...
  B[n-1].Vmin = minVol;
  B[n-1].Vmin_bus = busno;
    
  return B;  
}
int CheckConvergence(int cf, BusData* bus, LineData* line, Rect* LineLoss, Polar* V, SystemDetails s, BaseCaseResult* B, int n) {
  int i, j, busno, k;
  ...
  for(i = 1; i < s.nbus; i++) {
   ...
   if( (minVol > B[n-1].Vmin) && (SPL < B[n-1].Ploss)) {  // Voltage and Power Violations
     cf = 0;  
   } else
       cf = 1;
...
Result* initializeResultPointer(Result* Res, int n, SystemDetails s){
  int i, j;
  for(i = 0; i < n;i++) {
     Res[i].Ploss = 0;
     Res[i].Qloss = 0;
...
Result* CreateResultPointer(Result* Res, int FComb, SystemDetails s) {
  Res = (Result *)malloc(FComb*sizeof(Result));
  if(Res == NULL) {
    printf("\nError in memory allocation- Result pointer\n");  
    exit(1);
  }
  
  Res = initializeResultPointer(Res, FComb, s);
  return Res;
}
BaseCaseResult* initializeBCResultPointer(BaseCaseResult* B, int  n){
  B[n-1].Ploss = 0;
  B[n-1].Qloss = 0;
  B[n-1].Pload = 0;
  B[n-1].Qload = 0;
...
BaseCaseResult* CreateBaseCaseResultPointer( BaseCaseResult* B, int n) {
  B = (BaseCaseResult*)malloc(n*sizeof(BaseCaseResult));
  if(B == NULL) {
    printf("\nError in memory allocation- Result pointer\n");  
    exit(1);
  }
  
  B = initializeBCResultPointer(B, n);  
}
FILE* FileptrLoadFlowResults(FILE* fpo5, Result* Res, Rect* Volt, Rect* Sinj, BusData* bus, LineData* line, Rect** S, Rect* LineLoss, Polar* V, 
			     SystemDetails s, int aa, BaseCaseResult* B, int n, double dgvalue, int dgbusno, double LA) {
  int i, j, busno, k;
  ...
  if( (minVol > B[n-1].Vmin) && (SPL < B[n-1].Ploss)) {  // Voltage and Power Violations
     Res[aa].Ploss = SPL*1000;
     Res[aa].Qloss = SQL*1000;
     Res[aa].Pload = SumPload;
     Res[aa].Qload = SumQload;
     Res[aa].Vmin = minVol;
     Res[aa].Vmin_BusNo = busno;
     Res[aa].Pdg = dgvalue;
     Res[aa].Qdg = Res[aa].Pdg*tan(LA);
     Res[aa].dg_busno = dgbusno;
     Res[aa].Pgrid = ( (Res[aa].Ploss + Res[aa].Pload) - Res[aa].Pdg);
     Res[aa].Qgrid = ( (Res[aa].Qloss + Res[aa].Qload) - Res[aa].Qdg);
     fprintf(fpo5, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", Res[aa].Pdg, Res[aa].Qdg, Res[aa].Ploss, Res[aa].Qloss, Res[aa].Pload, Res[aa].Qload, Res[aa].Pgrid, Res[aa].Qgrid);
  } 
  
  return fpo5;
}

// **** Loss Sensitivity Factor Methods  ****
double** CalculateAlphaij(double** Alphaij, Admitance** ybus, Polar* V, SystemDetails s) {
  int i, j;
  double real, imag, num, angle;
  
  Alphaij = initializeAdjMatrixN(Alphaij, s.nbus);
  
  for(i = 0; i < s.nbus; i++) {
    for(j = 0; j < s.nbus; j++) {
      if(ybus[i][j].g != 0 && ybus[i][j].b != 0) {
	real = (ybus[i][j].g / (ybus[i][j].g*ybus[i][j].g + ybus[i][j].b*ybus[i][j].b));
	// printf("\nreal = %lf", real);
	num  = (real / (V[i].magni*V[j].magni));
	// printf("\tnum = %lf", num);
	angle = ( (V[i].theta-V[j].theta) * 180) / PI; 
	// printf("\tangle = %lf", angle);
	Alphaij[i][j] = num * cos(angle);
      }
    }
  }
...
double** CalculateBhetaij(double** Bhetaij, Admitance** ybus, Polar* V, SystemDetails s) {
  int i, j;
  double real, imag, num, angle;
  
  Bhetaij = initializeAdjMatrixN(Bhetaij, s.nbus);
  ...
  for(i = 0; i < s.nbus; i++) {
	...
	num  = (real / (V[i].magni*V[j].magni));
	// printf("\tnum = %lf", num);
	angle = ( V[i].theta-V[j].theta) ; 
	// printf("\tangle = %lf", angle);
	Bhetaij[i][j] = num * sin(angle);
      }
    }
  }
...
double* CalculateSensitivity(double* Alpha, Admitance** ybus, Polar* V, SystemDetails s, BusData* bus) {
  int i, j;
  double term, term1;
  
  double** Alphaij = createAdjMatrixN(Alphaij, s.nbus);
  double** Bhetaij = createAdjMatrixN(Bhetaij, s.nbus);
  // printAdjMatrixN(Bhetaij, s.nbus);
  
  Alphaij =  CalculateAlphaij(Alphaij, ybus, V, s);
  // printAdjMatrixN(Alphaij, s.nbus);
  
  Bhetaij = CalculateBhetaij(Bhetaij, ybus, V, s); 
  // printAdjMatrixN(Bhetaij, s.nbus);  
  ...
  for(i = 0; i < s.nbus; i++) {
    ...
    for(j = 0; j < s.nbus; j++) {
      term1 = 0;
      term1 = ( (Alphaij[i][j]*bus[j].Ps) - (Bhetaij[i][j]*bus[j].Qs));
      term = term + term1;
    }
    Alpha[i] = term;
  }
  ...
  for(i = 0; i < s.nbus; i++) {
    free(Alphaij[i]); free(Bhetaij[i]);
  } free(Alphaij); free(Bhetaij);
  return Alpha;
}
SensitivityFactor* initializeSenFactPointer(SensitivityFactor* SFact, int nb) {
  int i;
  for(i = 0; i < nb; i++) {
    SFact[i].bus_no = 0;
    SFact[i].Sens_Fact = 0;
...
SensitivityFactor* CreateSentivityPointer(SensitivityFactor* SFact, int nb) {
  SFact = (SensitivityFactor*)malloc(nb*sizeof(SensitivityFactor));
  if(SFact == NULL) {
    printf("\nError in memory Allocation - Sensitivity Factor");
    exit(1);
  }
  
  SFact = initializeSenFactPointer(SFact, nb);
  return SFact;
}
SensitivityFactor* CopyAlpha(SensitivityFactor* SFact, double* Alpha, int nb) {
  int i;
  
  for(i = 0; i < nb; i++){
    SFact[i].bus_no = i+1;
    SFact[i].Sens_Fact = Alpha[i];
  }
  
  return SFact;
}
void PrintSensitvityFactor(SensitivityFactor* SFact, int nbus) {
  int i;
  
  printf("\nBusno\t Sensitivity Factor\n");
  for(i = 0; i < nbus; i ++)
    printf("\n%d\t%lf", SFact[i].bus_no, SFact[i].Sens_Fact);
  
  printf("\n");
}
SensitivityFactor* SortingAlgorithm(SensitivityFactor* SFact, double* Alpha, int nbus) {
  int i, j, tindex;
  double temp;
  
  for(i = 0; i < nbus-1; i++) {
    for(j = 0; j < nbus-1; j++) {
      if(SFact[j].Sens_Fact < SFact[j+1].Sens_Fact) {
	tindex = SFact[j].bus_no;
	temp = SFact[j].Sens_Fact;
	SFact[j].bus_no = SFact[j+1].bus_no;
	SFact[j].Sens_Fact = SFact[j+1].Sens_Fact;
	SFact[j+1].bus_no = tindex;
	SFact[j+1].Sens_Fact = temp;
       }
     }
  }
...
int main(int argc, char* argv[]) {
  ...
  int i, Jsize, FComb, hrs, ldl, finc, one, noc, ii; 
  double Tol, BCLAng; 
  
  FComb = 10000; one = 1; cFlag = 0; noc = 50;
  hrs = 96; // 24 - WSSF 
  ...
  FILE* fp3 = FilePointerToRead(fp3, fpd.inputfile3);
  ...
  BaseCaseResult* BCRes = CreateBaseCaseResultPointer(BCRes, one);   // Base Case Results Pointer
  Result* Res = CreateResultPointer(Res, FComb, sys);       // Result Pointer
  
  double* Alpha = createDoubleArray(Alpha, sys.nbus);
  // PrintDoubleArray(Alpha, sys.nbus);
  
  SensitivityFactor* SFact = CreateSentivityPointer(SFact, sys.nbus);
    
  // -------------------------------------- Base Case Load Flow -----------------------------------------
  FILE* fbase = fopen("BaseCase.out", "w");
  ...
  BCRes =  ComputeBaseCaseResult(BCRes, bus, line, LineLoss,  V, sys, one);  
  
  fbase = BaseCaseFileWriting(fbase, BCRes, Volt, Sinj, bus, line, S, LineLoss, V, sys, one);
  
  printf("\n\nTest System consider is %d bus distribution system\n", sys.nbus);
  
  printf("\nBase Case Results :\n\nPLoss = %lf kW\t Qloss = %lf kVAr\tVmin (pu) = %lf", BCRes[one-1].Ploss, BCRes[one-1].Qloss, BCRes[one-1].Vmin);
  printf("\nTotal Pload = %lf kW\tTotal Qload = %lf kVAr\n", BCRes[one-1].Pload, BCRes[one-1].Qload);
  
  // BCLAng = BCRes[one-1].Qload / BCRes[one-1].Pload;
  // printf("\nQload/Pload = %lf", BCLAng);
  BCLAng = 0;
  
  printf("Load Angle is given as %lf (in radians)\n", BCLAng);
  
  Alpha = CalculateSensitivity(Alpha, ybus, V, sys, bus);
  // PrintDoubleArray(Alpha, sys.nbus);
  
  SFact = CopyAlpha(SFact, Alpha, sys.nbus);
  // printf("\nBefore Sorting\n");  PrintSensitvityFactor(SFact, sys.nbus);
  
  // SFact = SortingAlgorithm(SFact, Alpha, sys.nbus);
  // printf("\nAfter Sorting\n");  PrintSensitvityFactor(SFact, sys.nbus);
   
  // printf("\nBefore Modifying"); printBusData(tbus, sys.nbus);
  // -------------------------------------- Base Case LF Ends -----------------------------------------     
 
  fprintf(fpo5, "--------------------------------------------------------------------------------------------------------------------------------------\n");
  fprintf(fpo5, "Pdg(kW)\t\tQdg(kVAr)\tPLoss(kW)\tQLoss(kVAr)\tPLoad(kW)\tQLoad(kVAr)\tPgrid(kW)\tQgrid\tDG_Loc\tBus_no\tMin_Voltage");
  fprintf(fpo5, "\n--------------------------------------------------------------------------------------------------------------------------------------\n");
  double dgvalue, ini_dg_size; 
  int dgbusno; 
  
  finc = 0;
  for(ii = 0; ii < sys.nbus; ii++) {	 // limts are 0 to sys.nbus
    // printf("\nii = %d", SFact[ii].bus_no);
    ini_dg_size = 0;	
    do {
      if(ini_dg_size != 0) {
	tbus[SFact[ii].bus_no-1].Ps = tbus[SFact[ii].bus_no-1].Ps - ini_dg_size; 	// Increment in terms of 5 kW.
	tbus[SFact[ii].bus_no-1].Qs  = tbus[SFact[ii].bus_no-1].Qs - ini_dg_size*tan(BCLAng); 
      }
      else {
	tbus[SFact[ii].bus_no-1].Ps = tbus[SFact[ii].bus_no-1].Ps;
	tbus[SFact[ii].bus_no-1].Qs = tbus[SFact[ii].bus_no-1].Qs;
      }
   
      dgvalue = ini_dg_size;
      dgbusno = SFact[ii].bus_no;
      
      // printf("\n%lf\t\%lf", tbus[SFact[ii].bus_no-1].Ps, tbus[SFact[ii].bus_no-1].Qs);
      
      // printf("Dgsize = %lf\t@bus = %2d\t Real = %lf \t Reactive = %lf\n", dgvalue, dgbusno, tbus[SFact[ii].bus_no-1].Ps, tbus[SFact[ii].bus_no-1].Qs);
      ...
      Ppu = CalculatepuRealPower(Ppu, tbus, sys.nbus, sys.BaseMVA);   
      // printf("\nPerunit real power\n"); PrintDoubleArray(Ppu, sys.nbus);
        
      Qpu = CalculatepuReacPower(Qpu, tbus, sys.nbus, sys.BaseMVA); 
      // printf("\nPer Unit Reactive Power\n"); PrintDoubleArray(Qpu, sys.nbus); 
        
      V = LoadFlow(V, ybs, tbus, Ppu, Qpu, Volt, Pcal, Qcal, J1, J2, J3, J4, DelP, DelQ, J11, J22, J33, J44, Jsize, J, x, DelP1, DelQ1, fpo3, Jele, Sol, fpo2, fpo4, sys);
      // printf("\nAfter Convergence\n"); printDoubleArray(V, sys.nbus); 
      ...
      fpo5 = FileptrLoadFlowResults(fpo5, Res, Volt, Sinj, tbus, line, S, LineLoss, V, sys, finc, BCRes, one, dgvalue, dgbusno, BCLAng); 
          
      tbus = copybusPointerData(tbus, bus, sys);   // Reset of bus pointer
      // printBusData(tbus, sys.nbus);
      finc = finc + 1;
      ini_dg_size = ini_dg_size + 50;
    }while(ini_dg_size <=  DG_Max);
  } 
  // fprintf(fpo5, "--------------------------------------------------------------------------------------------------------------------------------------\n");
  
  fclose(fpo5); free(Res);
  
  // Determining Optimal Configuration Code
  char ch;
  int count, jj, index; 
  double temp;
  
  finc = 0; count = 0; index = 0;
  
  FILE* fp = fopen(fpd.output5,"r");
  if(fp == NULL) {
  ...
  Result* Opt_Conf = CreateResultPointer(Opt_Conf, FComb, sys);
  
  // Reading the entire file data
  for(jj = 0; jj < FComb; jj++) {
    fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf\n", &Opt_Conf[jj].Pdg, &Opt_Conf[jj].Qdg, &Opt_Conf[jj].Ploss, &Opt_Conf[jj].Qloss,
	   &Opt_Conf[jj].Pload, &Opt_Conf[jj].Qload, &Opt_Conf[jj].Pgrid, &Opt_Conf[jj].Qgrid);
    printf("\n%lf %lf %lf %lf %lf %lf %lf %lf\n", Opt_Conf[jj].Pdg, Opt_Conf[jj].Qdg, Opt_Conf[jj].Ploss, Opt_Conf[jj].Qloss,
	   Opt_Conf[jj].Pload, Opt_Conf[jj].Qload, Opt_Conf[jj].Pgrid, Opt_Conf[jj].Qgrid);
  }
  
  temp = BCRes[one-1].Ploss;
  for(jj = 0; jj < FComb-1; jj++) {
    if(Opt_Conf[jj].Ploss < temp) {
      // printf("%lf", Res[jj].Ploss);
      temp = Opt_Conf[jj].Ploss;
      index = jj;
    }
  } 
  printf("\nindex = %d\n", index);
  printf("%lf %lf %lf %lf %lf %lf %lf %lf\n", Opt_Conf[index].Pdg, Opt_Conf[index].Qdg, Opt_Conf[index].Ploss, Opt_Conf[index].Qloss,
	   Opt_Conf[index].Pload, Opt_Conf[index].Qload, Opt_Conf[index].Pgrid, Opt_Conf[index].Qgrid);
  
  
  
  
  //For Optimal Configuration Determination
  free(Opt_Conf); fclose(fp);
  // For Load Flow
  fclose(fp1); fclose(fbase); fclose(fp2); fclose(fp3);
  free(line); free(bus); free(tbus); 
  for(i = 0; i < sys.nbus; i++) {
  ...
  fclose(fpo1); fclose(fpo2); fclose(fpo3); fclose(fpo4); 
  free(Irect); 
  for(i = 0; i < sys.nbus; i++) {
    free(Iflow[i]); free(S[i]); } free(Iflow); free(S);
  free(LineLoss); free(Sinj); free(BCRes); 
