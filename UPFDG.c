/************************************************************************
*       Unity PF DG Sizing and Location using Analytic Methods		*
*								        * 
...
*									*
*  Last Modification Date : 27.04.2017					*
*************************************************************************/
...
FILE* BaseCaseFileWriting(FILE* fb, Result* Res, Rect* Volt, Rect* Sinj, BusData* bus, LineData* line, Rect** S, Rect* LineLoss, Polar* V, SystemDetails s) {
  ...
  fprintf(fb, "\nMinimum voltage in the %d bus system is at bus no : %d\tVoltage magnitude is : %lf" , s.nbus, busno, minVol);
...
BaseCaseResult* ComputeBaseCaseResult(BaseCaseResult* B, BusData* bus, LineData* line, Rect* LineLoss, Polar* V, SystemDetails s, int n) {
  double SumPload, SumQload, SumPLoss, minVol;
  int i, j, busno;
  
  SumPload = 0; SumQload = 0; SumPLoss = 0;  
...
int CheckConvergence(int cf, BusData* bus, LineData* line, Rect* LineLoss, Polar* V, SystemDetails s, BaseCaseResult* B, int n) {
  ...
  for(i = 1; i < s.nbus; i++) {
   ...
   if( (minVol > B[n-1].Vmin) && (SPL < B[n-1].loss)) {  // Voltage and Power Violations
     cf = 0;  
   } else
       cf = 1;
...
FILE* FileptrLoadFlowResults(FILE* fpo5, Result* Res, Rect* Volt, Rect* Sinj, BusData* bus, LineData* line, Rect** S, Rect* LineLoss, Polar* V, 
  ...
  if( (minVol > B[n-1].Vmin) && (SPL < B[n-1].loss)) {  // Voltage and Power Violations
     ...
     Res[aa].dgsize = dgvalue;
     Res[aa].dg_busno = dgbusno;
     
     fprintf(fpo5, "%lf\t%lf\t%lf\t%2d\t%2d\t%lf\n", Res[aa].dgsize, Res[aa].Ploss, Res[aa].dgsize*tan(LA), Res[aa].dg_busno, Res[aa].Vmin_BusNo, Res[aa].Vmin);
  } 
...
int main(int argc, char* argv[]) {
  ...
  fbase = BaseCaseFileWriting(fbase, Res, Volt, Sinj, bus, line, S, LineLoss, V, sys);
  
  BCRes =  ComputeBaseCaseResult(BCRes, bus, line, LineLoss,  V, sys, one);  
  printf("\n\nTest System consider is %d bus distribution system\n", sys.nbus);
  
  printf("\nBase Case Results :\n\nPLoss = %lf MW\t Vmin (pu) = %lf", BCRes[one-1].loss, BCRes[one-1].Vmin);
  ...
  BCLAng = atan(BCLAng);
  BCLAng = 0;	// For Unity Power Factor DG	
  ...
  fprintf(fpo5, "\n---------------------------------------------------------------------------------------\n");
  fprintf(fpo5, "DG_Size(P in kW)\tP_Loss\tQ(in kVAr)\tDG_Loc\tBus_no\tMin_Voltage");
  fprintf(fpo5, "\n---------------------------------------------------------------------------------------\n");
  // -------------------------------------- Base Case LF Ends -----------------------------------------     
  ...
  fclose(fpo1); fclose(fpo2); fclose(fpo3); fclose(fpo4); fclose(fpo5); 
  free(Irect); 
  for(i = 0; i < sys.nbus; i++) {
    free(Iflow[i]); free(S[i]); } free(Iflow); free(S);
  free(LineLoss); free(Sinj); free(BCRes); free(Res); 
